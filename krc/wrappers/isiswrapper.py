...
def preprocess_for_davinci(image, outcube, ckkernel=None, spkkernel=None):
    """
    ...
    if ckkernel is not None and spkkernel is not None:
        isis.spiceinit(from_=outcube, ck=ckkernel, spk=spkkernel)
    elif ckkernel is not None:
        isis.spiceinit(from_=outcube, ck=ckkernel)
    elif spkkernel is not None:
        isis.spiceinit(from_=outcube, spk=spkkernel)
    else:
        isis.spiceinit(from_=outcube)

def postprocess_for_davinci(incube, ckkernel=None, spkkernel=None):
    workingpath, fname = os.path.split(incube)
    fname = os.path.splitext(fname)[0]
    #Processing the temperature to a level2 image
    if ckkernel is not None and spkkernel is not None:
        isis.spiceinit(from_=incube, ck=ckkernel, spk=spkkernel)
    elif ckkernel is not None:
        isis.spiceinit(from_=incube, ck=ckkernel)
    elif spkkernel is not None:
        isis.spiceinit(from_=incube, spk=spkkernel)
    else:
